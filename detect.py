...
def arg_parse():
    ...
    parser.add_argument('--weights', dest='weightsfile', help='weightsfile',
                        default=os.path.join('weights', 'yolov3.weights'), type=str)
...
def main():
    ...
    for batch in im_batches:
        ...
        prediction = write_results(prediction, confidence, len(classes), nms = True, nms_conf = nms_thesh)
    ...
    def write(x, batches, results):
        c1 = (int(x[1].item()), int(x[2].item())) # tuple(x[1].int(), x[2].int())
        c2 = (int(x[3].item()), int(x[4].item())) # tuple(x[3:5].int())
        img = results[int(x[0])]
        print(f'c1: {c1}, c2: {c2}, img: {img.shape}')
        cls = int(x[-1])
